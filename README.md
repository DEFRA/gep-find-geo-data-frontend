...
Server-side session state is stored in Redis via [Catbox](https://hapi.dev/module/catbox/). Local development uses the Redis instance from Docker Compose so sessions survive server restarts. On CDP, Redis is namespaced per service (prefix: `gep-find-geo-data-frontend:`).
